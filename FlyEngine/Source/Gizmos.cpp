...
{
	// X Axis 
	axisXBox->CenterMinMaxPoints(); 
	float2 moveMaxPoint = float2(axisXBox->GetMaxPoint().x + lineLength / 2, axisXBox->GetMaxPoint().y);
	float2 moveMinPoint = float2(axisXBox->GetMinPoint().x + lineLength / 2, axisXBox->GetMinPoint().y);
